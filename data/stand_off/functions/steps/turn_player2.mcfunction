...
execute as @e[type=fishing_bobber, nbt={Motion:[0d,0d,0d]}, tag=standoff, tag=player2] at @s unless entity @e[type=minecraft:armor_stand, tag=standoff, tag=player2, sort=nearest, distance=..2] run kill @s
tag @e[type=fishing_bobber, distance=..2] add standoff
tag @e[type=fishing_bobber, distance=..2, tag=standoff] add player2
