...
execute if entity @e[type=minecraft:armor_stand, tag=standoff, tag=player2, tag=landing] if score @s PlacedPawn matches 1.. unless score @s double_move matches 2 run function stand_off:next_turn
execute if entity @e[type=minecraft:armor_stand, tag=standoff, tag=player2, tag=landing] if score @s PlacedPawn matches 1.. unless score @s double_move matches 2 run scoreboard players operation $current standoff_state = $turn_player1 standoff_state
execute if entity @e[type=minecraft:armor_stand, tag=standoff, tag=player2, tag=landing] if score @s PlacedPawn matches 1.. if score @s double_move matches 2 run scoreboard players add @s double_move 1
