...
execute at @e[type=minecraft:armor_stand, tag=landed] if data storage stand_off:storage steps.current.is_player1 store result score removed PlacedPawn run kill @e[type=minecraft:armor_stand, tag=player2, distance=..0.5]
execute at @e[type=minecraft:armor_stand, tag=landed] unless data storage stand_off:storage steps.current.is_player1 store result score removed PlacedPawn run kill @e[type=minecraft:armor_stand, tag=player1, distance=..0.5]

execute if data storage stand_off:storage steps.current.is_player1 run scoreboard players operation @e[tag=player1, tag=standoff] PlacedPawn -= removed PlacedPawn
execute unless data storage stand_off:storage steps.current.is_player1 run scoreboard players operation @e[tag=player2, tag=standoff] PlacedPawn -= removed PlacedPawn
...
execute if entity @e[type=minecraft:armor_stand, tag=landed, tag=player2] unless data storage stand_off:storage steps.current.is_player1 run data merge storage stand_off:storage {steps:{current:{is_player1:1b}}}
